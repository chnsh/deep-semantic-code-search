...
class RNN_Encoder(nn.Module):
    ...
    def __init__(self, ntoken, emb_sz, nhid, nlayers, pad_token, bidir=False,
        ...
        self.rnns = [
            nn.LSTM(emb_sz if l == 0 else nhid, (nhid if l != nlayers - 1 else emb_sz) // self.ndir,
...
def get_rnn_classifer(bptt, max_seq, n_class, n_tok, emb_sz, n_hid, n_layers, pad_token, layers,
                      drops, bidir=False,
                      dropouth=0.3, dropouti=0.5, dropoute=0.1, wdrop=0.5):
    rnn_enc = MultiBatchRNN(bptt, max_seq, n_tok, emb_sz, n_hid, n_layers, pad_token=pad_token,
                            bidir=bidir,
                            dropouth=dropouth, dropouti=dropouti, dropoute=dropoute, wdrop=wdrop)
