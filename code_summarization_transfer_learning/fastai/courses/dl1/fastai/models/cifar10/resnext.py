...
class ResNeXtBottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, cardinality, base_width, stride=1, downsample=None):
        ...
        C = cardinality
        self.conv_reduce = nn.Conv2d(inplanes, D * C, kernel_size=1, stride=1, padding=0,
                                     bias=False)
        self.bn_reduce = nn.BatchNorm2d(D * C)

        self.conv_conv = nn.Conv2d(D * C, D * C, kernel_size=3, stride=stride, padding=1,
                                   groups=cardinality, bias=False)
        self.bn = nn.BatchNorm2d(D * C)
        self.conv_expand = nn.Conv2d(D * C, planes * 4, kernel_size=1, stride=1, padding=0,
                                     bias=False)
...
class CifarResNeXt(nn.Module):
    ...
    def _make_layer(self, planes, stride=1):
        ...
        layers = []
        layers.append(self.block(self.inplanes, planes, self.cardinality, self.base_width, stride,
                                 downsample))
