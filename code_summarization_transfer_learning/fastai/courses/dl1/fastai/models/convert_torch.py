...
def lua_recursive_model(module, seq):
    for m in module.modules:
        ...
        if name == 'SpatialConvolution':
            ...
            n = nn.ConvTranspose2d(m.nInputPlane, m.nOutputPlane, (m.kW, m.kH), (m.dW, m.dH),
                                   (m.padW, m.padH))
            add_submodule(seq, n)
...
def lua_recursive_source(module):
    ...
    for m in module.modules:
        ...
        if name == 'SpatialConvolution':
            ...
            s += ['nn.BatchNorm2d({},{},{},{}),#BatchNorm2d'.format(m.running_mean.size(0), m.eps,
                                                                    m.momentum, m.affine)]
        elif name == 'ReLU':
            s += ['nn.ReLU()']
        elif name == 'SpatialMaxPooling':
            ...
            s2 = 'nn.Linear({},{},bias={})'.format(m.weight.size(1), m.weight.size(0),
                                                   (m.bias is not None))
            ...
            dim = m.dimension
            s += ['LambdaReduce(lambda x,y,dim={}: torch.cat((x,y),dim), # Concat'.format(
                m.dimension)]
            s += lua_recursive_source(m)
            s += [')']
        else:
