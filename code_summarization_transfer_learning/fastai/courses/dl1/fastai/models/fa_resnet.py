    ...
    def __init__(self, block, layers, num_classes=1000, k=1, vgg_head=False):
        ...
        if vgg_head:
            features += [nn.AdaptiveAvgPool2d(3), Flatten()
                , nn.Linear(out_sz * 3 * 3, 4096), nn.ReLU(inplace=True), bn1(4096),
                         nn.Dropout(0.25)
...
def fa_resnet18(pretrained=False, **kwargs):  return load(
    ResNet(BasicBlock, [2, 2, 2, 2], **kwargs), pretrained, 'resnet18')


def fa_resnet34(pretrained=False, **kwargs):  return load(
    ResNet(BasicBlock, [3, 4, 6, 3], **kwargs), pretrained, 'resnet34')


def fa_resnet50(pretrained=False, **kwargs):  return load(
    ResNet(Bottleneck, [3, 4, 6, 3], **kwargs), pretrained, 'resnet50')


def fa_resnet101(pretrained=False, **kwargs): return load(
    ResNet(Bottleneck, [3, 4, 23, 3], **kwargs), pretrained, 'resnet101')


def fa_resnet152(pretrained=False, **kwargs): return load(
